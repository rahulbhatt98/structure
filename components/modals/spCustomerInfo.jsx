...
function SpCustomerInfo({ isOpen, onClose, markers, id, handleNavigation }) {
    const matchedMarker = markers.find(marker => (marker.request_details?.user_details?.user_id ? marker?.request_details?.user_details?.user_id : marker?.user_details?.user_id) === id);
                        ...
                        <div className="w-[90%] xl:w-2/3 mr-3 md:mr-0 ">
                            <div className="flex flex-wrap justify-between">
                                <h3 className="text-base md:text-lg font-semibold">
                                    {matchedMarker?.request_details?.user_details?.first_name ? matchedMarker?.request_details?.user_details?.first_name : matchedMarker?.user_details?.first_name} {matchedMarker?.request_details?.user_details?.last_name ? matchedMarker?.request_details?.user_details?.last_name : matchedMarker?.user_details?.last_name}
                                </h3>
                            ...
                            <div className="flex items-center flex-wrap justify-between text-xs md:text-sm text-gray-600">
                                <div className='flex gap-3 items-center pt-2'>
                                    ...
                                    <span className="text-xs md:text-sm mainbluetext font-semibold">
                                        {(matchedMarker?.request_details?.distance ? matchedMarker?.request_details?.distance : matchedMarker?.user_details?.distance ? matchedMarker?.user_details?.distance : 0.00).toFixed(1)} Km{' '}
                            ...
                            <div className="flex items-center py-2 -ml-1">
                                ...
                                <p className="text-xs md:text-sm">
                                    {matchedMarker?.request_details?.address ? matchedMarker?.request_details?.address : matchedMarker?.address}
                                </p>
                            </div>
                            {/* <p className="text-xs md:text-sm text-[#919EAB] ml-1">
                                Profession: <span className="text-black">{matchedMarker?.request_details?.RequestServices[0]?.services?.categories?.name || 'N/A'}</span>
                            </p> */}
                        </div>
                        <div className="relative">
                            <Image
                                src={matchedMarker?.request_details?.user_details?.profile_photo ? matchedMarker?.request_details?.user_details?.profile_photo :  '/assets/professional1.png'}
                                alt="user"
                                width="131"
                                height="131"
