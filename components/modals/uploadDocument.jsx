...
function UploadDocument({ isOpen, onClose, selectedDocument, setSelectedServices, selectedServices, setdocsError }) {
        ...
        dispatch(uploadTheDocuments(formData))
            ...
            .then((obj) => {
                ...
                if (type === "license") {
                    ...
                    if (status) {
                        const updatedArray = licensesUrls.map(obj =>
                            obj.id === matchUrl ? uploadedUrl[0] : obj
                        );
                        const updatedServices = updateServiceImage(checkedServices, selectedId, updatedArray);
                        console.log(checkedServices, selectedId, updatedArray, updatedServices, "assssssssssssssssss");
                        setLicensesUrls(updatedArray);
                        setCheckedService(updatedServices)
                    }
                    else {
                ...
                if (type === "certificate") {
                    ...
                    if (status) {
                        const updatedArray = certificatesUrls.map(obj =>
                            obj.id === matchUrl ? uploadedUrl[0] : obj
                        );
                        const updatedServices = updateServiceImage(checkedServices, selectedId, updatedArray);
                        console.log(checkedServices, selectedId, updatedArray, updatedServices, "assssssssssssssssss");
                        setCertificatesUrls(updatedArray);
                        setCheckedService(updatedServices)
                    }
                    else {
        ...
        <>
            <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
                <div className="relative bg-white w-full max-w-md rounded-2xl shadow-lg mx-4">
                    ...
                    <div className='h-96 mx-2 overflow-y-auto border'>
                        ...
                        <div className=" p-1 rounded-lg mt-3">
                            ...
                            {(selectedId && (checkedServices?.find(val => val?.id == selectedId) && checkedServices?.find(val => val?.id == selectedId)?.license_required === true)) &&
                                <div>
                                    ...
                                    <div className="flex flex-wrap gap-4">
                                                ...
                                                licensesUrls?.map((file, index) => (
                                                    ...
                                                    >
                                                        ...
                                                        {((!file?.type ? file?.image?.toLowerCase()?.slice(-3) === 'pdf' : false) || file?.type === "application/pdf") ? (
                                                            ...
                                                            <div className=' '>
                                                                {(auth?.profile_completed_steps === "4" && file?.status === "refuse") && <div className="absolute inset-0 rounded-xl bg-black bg-opacity-10 backdrop-blur-sm"></div>}
                                                                <Image
                                                                    src="/assets/pdf.svg"
                                                                ...
                                                                />
                                                                {(auth?.profile_completed_steps === "4" && file?.status === "refuse") && <label className="">
                                                                    <Image
                                                                        width="50"
                                                                        height="50"
                                                                        src="/assets/uploaddocs.svg"
                                                                        alt="Experience Certificate Icon"
                                                                        className="absolute top-1/3 left-1/3 cursor-pointer"
                                                                    />
                                                                    <input
                                                                        type="file"
                                                                        style={{ display: "none" }}
                                                                        accept=".jpg, .png, .pdf, .jpeg"
                                                                        onChange={(e) => handleLicenseChange(e, "license", (file?.status === "refuse"), file?.id)}
                                                                    // multiple
                                                                    // maxLength={3}
                                                                    />
                                                                </label>}
                                                        ...
                                                        {console.log(auth?.profile_completed_steps, "sadsasasasa")}
                                        ...
                                        {(licensesUrls?.length < 3 && auth?.profile_completed_steps !== "4") && (
                            ...
                            {(selectedId && (checkedServices?.find(val => val?.id == selectedId) && checkedServices?.find(val => val?.id == selectedId)?.certificate_required === true)) && <div>
                                ...
                                <div className="flex flex-wrap gap-4">
                                                        ...
                                                        <div className=' '>
                                                            ...
                                                            {(auth?.profile_completed_steps === "4" && file?.status === "refuse") && <label className="">
                                                                ...
                                                                <input
                                                                    ...
                                                                    style={{ display: "none" }}
                                                                    accept=".jpg, .png, .pdf, .jpeg"
                                                                    onChange={(e) => handleCertificatesChange(e, "certificate", (file?.status === "refuse"), file?.id)}
                                                                // multiple
                                                                // maxLength={3}
                                                                />
                                                            </label>}
                                    ...
                                    {(certificatesUrls?.length < 3 && auth?.profile_completed_steps !== "4") && (
                                        <label className="flex servicesShadow bg-white w-36 h-36 cursor-pointer flex-col justify-center items-center p-6 border border-dashed border-[#8DB7DD] rounded-xl">
                                            <Image
                                                width="50"
                                                height="50"
