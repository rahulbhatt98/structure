...
import { addProfessionalProfileAsync, getServiceStepAsync, getTheProfile, listofService, selectLoginAuth, setupProfessionalProfile } from '../../redux/auth/authSlice';
...
import UploadDocument1 from '../modals/uploadDocument1';

function Services1({ setStepValue }) {
    ...
    const [filteredServicesDocs, setFilteredServicesDocs] = useState("")

    const auth = useSelector(selectLoginAuth);
    const defaultServices = auth?.professional?.professionalUserServices || [];
    console.log(auth?.professional?.professionalUserServices, "ssssssssssssaaaaaaaaaaaaaaaaaaawqqqqqqqqqqqqqqqqqqqq");
    ...
    const adminVerified = auth?.professional?.professionalUserServices?.filter(val => val?.admin_status == "approve")
    ...
    console.log(uniqueCategoryIds, "default service");
    ...
    const serviceIdsToRemove = adminVerified?.map(item => item?.service_id);

    const filteredDataServiceList = (serviceList && serviceList?.length > 0) &&serviceList.map(category => {
        // Filter out the services in the outer service array
        const filteredServices = category?.service?.filter(service => !serviceIdsToRemove?.includes(service.id));

        // Filter out the subcategories
        const filteredSubCategories = category.subCategories.map(subCategory => {
            return {
                ...subCategory,
                service: subCategory?.service?.filter(service => !serviceIdsToRemove?.includes(service.id))
            };
        });

        // Return the category with the filtered services and subcategories
        return {
            ...category,
    ...
    const handleCheckboxChange = (serviceId, service, checkedValue) => {
        ...
        console.log(updatedServices, updatedServices?.filter((val) => val?.category_id == serviceId), "u[pdateeeeeeeeeee dserviceeeeeeeeeee");
    ...
    const handleCheckboxChangeService = (serviceId, categoryId, serviceData) => {
        ...
        // const updatedServices1 = selectedServices?.find(val => val?.category_id == categoryId)?.services?.filter(
        //     (service) =>
        //       (service.license_required || service.certificate_required) &&
        //       service.checked
        //   );
        //   console.log(updatedServices1,"ssssssssssssssssssssaaaaaaaaa");

        // let filteredData = selectedServices.map(category => {
        //     if (category.category_id === categoryId) {
        //         return {
        //             ...category,
        //             services: updatedServices
    ...
    const handleCheckboxChangeSubcategory = (categoryId, subcategoryId, subcategory) => {
        ...
        const services = filteredDataServiceList.flatMap(service =>
    ...
    const handleSave = (categoryId, subCategories) => {
        console.log(selectedServices?.find(val => val?.category_id == categoryId)?.services, "sssssssssssssss");
        ...
        if (selectedSubcategoriesIds.length > 0 || selectedServiceIds.length > 0) {
            ...
            console.log(selectedServices, categoryId, selectedServiceDetails, "selected serviceeeeeeeeeeeeee");
    ...
    const setupProfileFunc = () => {
        ...
        if (param?.serviceDetail?.length === 0) {
            if (!toast.isActive(toastId.current)) {
                toastId.current = toast.error("Please select atleast one service");
            }
            return
        }
        setIsLoading(true);
        dispatch(addProfessionalProfileAsync(param))
            .then(unwrapResult)
            .then((obj) => {
                if (!toast.isActive(toastId.current)) {
    ...
    console.log(checkedSubcategories, "aaaaaaaaaaaqqqqqqqqqqqqqqqqq");
                    ...
                    <div className='profileHeight PersonalDetails pb-5 overflow-y-auto'>
                        {
                            serviceList.length > 0 ?
                                <ul className='mt-3 space-y-3'>
                                    {filteredDataServiceList?.map((listItem, index) => (
                                        <li key={index}>
                                            <div className='servicesShadow flex justify-between pr-2 medmob:pr-6 bg-white h-20'>
                                                ...
                                                <div className='mt-3'>
                                                    ...
                                                    <input type="checkbox" className="checkbox-custom2 cursor-pointer" checked={openServiceIds.includes(listItem.id)} onChange={(e) => { handleCheckboxChange(listItem.id, listItem?.service, e.target.checked); setnoServiceError("") }} />
                                                </div>
                                            </div>
                                            ...
                                            {(isEditing || isSaved) && openServiceIds.includes(listItem.id) && (openServiceSelectedId == listItem?.id) && (
                                                ...
                                                <div className='p-4 bg-white serviceBox'>
                                                    ...
                                                    {console.log(selectedServices, "selected swrvcisssssssssssss")}
                                                    {
                                                        (
                                                    ...
                                                    <div className='flex gap-4 md:justify-end mt-4 md:mt-0'>
                                                        ...
                                                        {console.log(listItem?.service, listItem, selectedServices, serviceList, serviceList?.filter(val => val?.id === listItem?.id), selectedServices, "222222222222222222222222222222")}
                                                        ...
                                                        <button
                                                            className='lg:mt-4 py-2 px-2 md:px-6 blueBorder rounded-xl bg-white mainbluetext text-sm md:text-base'
